...
void main() {
  ...
  late ApzNetworkState apzNetworkState;
  ...
  setUp(() {
    ...
    apzNetworkState = ApzNetworkState();
    ...
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(
      ...
      (MethodCall methodCall) async {
        if (methodCall.method == 'requestPermissions') {
          ...
          return {Permission.location.value: PermissionStatus.granted.index}; // 1 for granted
        }
        return null;
      },
    );
  ...
  group('ApzNetworkState', () {
    ...
    test('getNetworkState throws PermissionException when location permission is denied', () async {
      // Arrange
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(
        permissionChannel,
        (MethodCall methodCall) async {
          if (methodCall.method == 'requestPermissions') {
            return {Permission.location.value: PermissionStatus.denied.index}; // 0 for denied
          }
          return null;
        },
      );
      ...
      // Act & Assert
      // Catch the exception and then assert its properties
      try {
        ...
        expect(e.message, "Location permission not granted.");
      } catch (e) {
        fail('Expected PermissionException, but caught ${e.runtimeType}: $e');
      }
    });

    test('getNetworkState throws PermissionException when location permission is permanently denied', () async {
      // Arrange
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(
        permissionChannel,
        (MethodCall methodCall) async {
          if (methodCall.method == 'requestPermissions') {
            return {Permission.location.value: PermissionStatus.permanentlyDenied.index}; // 2 for permanentlyDenied
          }
          return null;
        },
      );
      ...
      try {
        ...
        expect(e.message, "Location permission permanently denied. Please enable it from settings.");
      } catch (e) {
        fail('Expected PermissionException, but caught ${e.runtimeType}: $e');
      }
    });

    test('getNetworkState throws PermissionException when location permission is restricted', () async {
      // Arrange
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger.setMockMethodCallHandler(
        permissionChannel,
        (MethodCall methodCall) async {
          if (methodCall.method == 'requestPermissions') {
            return {Permission.location.value: PermissionStatus.restricted.index}; // 3 for restricted
          }
          return null;
        },
      );
      ...
      try {
        ...
        expect(e.message, "Location access restricted or not fully granted. Please check your device settings.");
      } catch (e) {
        fail('Expected PermissionException, but caught ${e.runtimeType}: $e');
      }
    });
