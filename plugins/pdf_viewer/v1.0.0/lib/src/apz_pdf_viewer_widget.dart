...
class ApzPdfViewer extends StatefulWidget {
  ...
  /// Optional builder to provide a custom [PdfViewer] widget, typically for testing.
...
class ApzPdfViewerState extends State<ApzPdfViewer> {
  ...
  @visibleForTesting
void setShowErrorText(final bool value) {
  setState(() {
...
PdfViewerParams buildPdfViewerParams() => PdfViewerParams(
    ...
    viewerOverlayBuilder: (
      BuildContext context,
      Size size,
      bool Function(Offset) handleLinkTap,
    ) =>
        <Widget>[
          GestureDetector(
            behavior: HitTestBehavior.translucent,
            onLongPress: () async => await widget._controller.resetZoom(),
            onDoubleTap: () async => await widget._controller.zoomUp(loop: true),
            onTapUp: (TapUpDetails details) => handleLinkTap(details.localPosition),
          ...
          PdfViewerScrollThumb(
            ...
            thumbBuilder: (context, size, pageNumber, controller) => ColoredBox(
  ...
  Widget build(final BuildContext context) => LayoutBuilder(
    ...
    // Use the injected builder if provided, otherwise create the real PdfViewer
    try{
    if (widget._pdfViewerBuilder != null) {
      viewer = widget._pdfViewerBuilder!(
        widget._source,
        widget._sourceType,
  ...
  /// @nodoc
  /// This method is exposed for testing purposes only to trigger the password dialog.
  @visibleForTesting
