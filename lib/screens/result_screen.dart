...
class ResultScreen extends StatelessWidget {
  ...
  Widget _buildSuccessContent(BuildContext context) {
    ...
    switch (plugin.name) {
      case 'camera':
      case 'photopicker':
        if (result is Map && (result['imagePath'] != null || result['filePath'] != null)) {
          ...
          content = SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                _buildInfoRow('Image Path', result['imagePath']?.toString() ?? result['filePath']?.toString() ?? 'N/A'),
                _buildInfoRow('Base64 Size (KB)', result['base64SizeKB']?.toString() ?? result['fileSizeKB']?.toString() ?? 'N/A'),
                if (base64 != null)
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
...
case 'apz_speech_to_text':
          ...
          content = StreamBuilder<Map<String, dynamic>>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasData) {
                ...
                return Column(
                  children: [
                    Text('Recognized text: ${data['text']}'),
                    if (logs != null && logs.isNotEmpty)
                      LogView(logs: logs),
                  ],
                );
              }
              return const Text('No speech data');
            },
          );
        } else {
          content = const Text('Speech to text result not available');
        }
        break;
        ...
        content = Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  result['result'] ?? 'Text to speech operation completed.',
                  style: const TextStyle(fontSize: 16),
                ),
              ),
            ],
      ...
      case 'apz_audioplayer':
        content = Container(
          ...
          child: Row(
            children: [
              Icon(Icons.audiotrack, color: Colors.orange[600]),
              const SizedBox(width: 12),
              Expanded(
                child: Text(
                  result['result'] ?? 'Audio player operation completed.',
                  style: const TextStyle(fontSize: 16),
                ),
              ),
            ],
          ...
          content = StreamBuilder<Map<String, dynamic>>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasData) {
                ...
                return Column(
                  children: [
                    Text('Received SMS: ${data['sms']}'),
                    if (logs != null && logs.isNotEmpty)
                      LogView(logs: logs),
                  ],
                );
              }
              return const Text('No SMS data');
            },
          );
        } else {
          content = const Text('Auto read OTP result not available');
        }
        break;
          ...
          content = StreamBuilder<Map<String, dynamic>>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasData) {
                ...
                return Column(
                  children: [
                    Text('Call state: ${data['callState']}'),
                    if (logs != null && logs.isNotEmpty)
                      LogView(logs: logs),
                  ],
                );
              }
              return const Text('No call state data');
            },
          );
        } else {
          content = const Text('Call state result not available');
        }
        break;

      case 'apz_charts':
        if (result is Map && result.containsKey('widget')) {
          content = result['widget'];
        } else {
          content = const Text('Chart widget not available');
        }
        break;
      default:
        content = Container(
